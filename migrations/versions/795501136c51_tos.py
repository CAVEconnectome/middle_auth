...
def upgrade():
    ...
    ### create flywire tos
    op.execute(
        "INSERT INTO \"tos\" (name, text) values ('flywire', 'I have read and agree to FlyWire''s [Principles](https://flywire.ai/principles.html) and [Terms of Service](https://flywire.ai/tos.html), including the use of cookies to maintain user login.')"
    )

    ### copy gdpr_consent into user_tos for flywire
    op.execute(
        'insert into user_tos (user_id, tos_id) select id, 1 from "user" where gdpr_consent = true'
    )
