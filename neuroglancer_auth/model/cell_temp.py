...
class CellTemp(db.Model):
    ...
    def is_public(table_id, root_id):
        query = CellTemp.query.filter_by(
            table_id=table_id, root_id=root_id, public=True
        ).exists()
    ...
    def get_public_roots(table_id):
        query = CellTemp.query.filter_by(table_id=table_id, public=True)
        return [r.root_id for r in query.all()]
