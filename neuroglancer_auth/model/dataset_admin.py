...
class DatasetAdmin(db.Model):
    ...
    def is_dataset_admin(user_id, dataset_id):
        query = DatasetAdmin.query.filter_by(
    ...
    def get_all_by_admin(user_id):
        datasets = (
            ...
            .filter(DatasetAdmin.user_id == user_id)
            .all()
        )
    ...
    def get_all_by_dataset(dataset_id):
        users = (
            db.session.query(DatasetAdmin.user_id, User.name)
            .filter(DatasetAdmin.user_id == User.id)
            .filter(DatasetAdmin.dataset_id == dataset_id)
            .all()
        )
