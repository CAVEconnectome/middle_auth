...
def setPermissionLookupOverride(func):
    print("setPermissionLookupOverride")
...
def get_user_cache(token):
    print("get_user_cache")
    lookup_function = (
        _permission_lookup_override if _permission_lookup_override else user_cache_http
    )

    print(f"lookup_function: {lookup_function}")
    res = lookup_function(token)
    print(f"lookup_function res: {res}")
    return res
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            def has_permission(auth_user):
                res = required_permission in auth_user.get("permissions_v2", {}).get(
                    local_dataset, []
                )

                if "permissions_v2" not in auth_user:  # backwards compatability
            ...
            if (
                AUTH_DISABLED
                or has_permission(flask.g.auth_user)
