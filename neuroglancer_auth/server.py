...
from middle_auth_client import (
    ...
    auth_requires_admin,
    make_api_error,
    setPermissionLookupOverride,
)
from oauthlib import oauth2
from yarl import URL

from .model.api_key import (
    APIKey,
    delete_all_temp_tokens_for_user,
    delete_token,
    get_redis_cache,
)
...
api_v1_bp = flask.Blueprint(
    "api_v1_bp", __name__, url_prefix="/" + URL_PREFIX + "/api/v1"
)
authorize_bp = flask.Blueprint(
    "authorize_bp", __name__, url_prefix="/" + URL_PREFIX + "/api/v1"
)
admin_site_bp = flask.Blueprint(
    "admin_site_bp", __name__, url_prefix="/" + URL_PREFIX + "/admin"
)
user_settings_bp = flask.Blueprint(
...
def requires_dataset_admin(f):
    ...
    def decorated_function(dataset_id, *args, **kwargs):
        is_dataset_admin = flask.g.auth_user["admin"] or DatasetAdmin.is_dataset_admin(
            flask.g.auth_user["id"], dataset_id
        )
...
def requires_group_admin(f):
    ...
    def decorated_function(group_id, *args, **kwargs):
        is_group_admin = flask.g.auth_user["admin"] or UserGroup.is_group_admin(
            flask.g.auth_user["id"], group_id
        )
...
def authorize():
    ...
    flow.redirect_uri = flask.url_for(
        "authorize_bp.oauth2callback", _external=True, _scheme="https"
    )
    ...
    programmatic_access = flask.request.headers.get(
        "X-Requested-With"
    )  # or cors_origin
...
def finish_auth_flow(token, template_name=None, template_context={}):
    ...
    if redirect:
        return redirect_with_args(
            redirect,
            token,
            {
                TOKEN_NAME: token,
                "middle_auth_url": STICKY_AUTH_URL,
...
def maybe_handle_tos(user, token, template_name=None, template_context={}):
    ...
    if tos_id:
        return redirect_with_args(
            flask.url_for("authorize_bp.tos_accept_view", tos_id=tos_id), token
        )
    else:
...
def oauth2callback():
    if "session" not in flask.request.cookies:
    ...
    if new_account:
        user = User.create_account(
            info["email"], info["name"], None, False, False, group_names=["default"]
        )
    else:
    ...
    if new_account:
        return redirect_with_args(
            flask.url_for("user_settings_bp.register_choose_username_view"),
            token,
            {"new_account": "true"},
        )
    else:
...
def get_users_by_filter():
    ...
    if flask.request.args.get("id"):
        users = User.filter_by_ids(
            [
                int(x)
                for x in flask.request.args.get("id").split(",")
                if x and x.isdigit()
            ]
        )
        ...
        users = User.filter_by_created(
            flask.request.args.get("from"), flask.request.args.get("to")
        )
    else:
...
def get_usernames():
    ...
    if flask.request.args.get("id"):
        users = User.filter_by_ids(
            [int(x) for x in flask.request.args.get("id").split(",") if x]
        )
...
def create_user_route():
    ...
    try:
        user = User.create_account(
            data["email"], data["name"], None, False, False, group_names=["default"]
        )
...
@api_v1_bp.route(
    "/user/token", methods=["POST"]
)  # should it be a post if there is no input data?
@auth_required
def create_token():
...
def refresh_token():
    ...
    if num_of_keys > 1:
        return flask.Response(
            "Refresh token does not work for accounts with more than one API Key", 400
        )
...
@requires_dataset_admin
def get_all_groups_for_dataset(
    dataset_id,
):  # nearly identical to get_datasets_from_group_route
    ...
    if "group_id" in data:
        ...
        try:
            GroupDatasetPermission.add(
                dataset_id=dataset_id, group_id=group_id, permission_ids=permission_ids
            )
            return flask.jsonify("success")
        except sqlalchemy.exc.IntegrityError:
            return flask.make_response(
                flask.jsonify("Dataset already includes group with permission."), 422
            )
    else:
...
@api_v1_bp.route(
    "/dataset/<int:dataset_id>/group/<int:group_id>/permission/<int:permission_id>",
    methods=["DELETE"],
)
@requires_dataset_admin
def remove_dataset_to_group_route(dataset_id, group_id, permission_id):
    ...
    )  # TODO return error if group doesn't exist
    return flask.jsonify("success")
...
@api_v1_bp.route(
    "/group/<int:group_id>/service_account/<int:sa_id>", methods=["DELETE"]
)
...
def get_service_accounts_by_filter():
    ...
    if flask.request.args.get("id"):
        service_accounts = User.filter_by_ids(
            [int(x) for x in flask.request.args.get("id").split(",") if x]
        )
    elif flask.request.args.get("name"):
...
def create_service_account_route():
    ...
    try:
        sa = User.create_account(
            data["name"].lower() + "@serviceaccount",
            data["name"],
            None,
            False,
            False,
            group_names=["default"],
            parent_id=flask.g.auth_user["id"],
        )
...
@user_settings_bp.route("/username", methods=["GET"])
@api_v1_bp.route("/register/choose_username", methods=["GET"])
@auth_required
...
@user_settings_bp.route("/username", methods=["POST"])
@api_v1_bp.route("/register/choose_username", methods=["POST"])
@auth_required
def register_choose_username_post():
    ...
    template_context = {"title": "User Settings Updated"}
    ...
    if form_custom:
        ...
        if not same_custom and not UserCustomName.add(user.id, form_custom):
            return flask.render_template(
                "username.jinja",
                user=user,
                prior=prior_custom.name,
                failure="You cannot change your custom name.",
            )
        else:
            if same_custom:
        ...
        template_context["msg"] = (
            "Your username was reset to the one associated with your Google Account"
        )
...
@authorize_bp.route("/tos/<int:tos_id>/accept", methods=["GET"])
@auth_required
def tos_accept_view(tos_id):
    ...
    flask.session["redirect"] = flask.request.args.get("redirect")
    if not tos:
        return flask.Response("Terms of Service does not exist", 404)
    user_id = flask.g.auth_user["id"]
    existing = UserTos.get(user_id, tos_id)
    if existing:
        return flask.render_template(
            "msg.jinja",
            title=f"{tos.name}'s Terms of Service",
            msg="You have already accepted the Terms of Service",
        )
    else:
        return flask.render_template("tos-form.html", name=tos.name, text=tos.text)


@authorize_bp.route("/tos/<int:tos_id>/accept", methods=["POST"])
@auth_required
def tos_accept_post(tos_id):
    ...
    try:
        ...
        if is_neuroglancer:
            ...
            return finish_auth_flow(
                flask.g.auth_token,
                "msg.html",
                {
                    "title": f"{tos.name}'s Terms of Service",
                    "msg": "Thank you for accepting the Terms of Service!",
                },
            )
    except sqlalchemy.exc.IntegrityError:
...
generic_post(
    api_v1_bp, "dataset", Dataset, prefix="/dataset", required_fields=["name", "tos_id"]
)
...
@api_v1_bp.route(
    "/service/<service>/table/<table>/dataset/<dataset>", methods=["DELETE"]
)
@auth_requires_admin
